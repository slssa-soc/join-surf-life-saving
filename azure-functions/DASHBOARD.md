...
Delivery settings now includes independent monthly-report recipients (up to 50), an enable switch and a separate test address. Reports start disabled. Save addresses before enabling. Reports cover the previous Adelaide calendar month and are due from 9 am on the first; the UTC timer checks every half-hour to handle daylight saving. If the app is unavailable throughout that day, the report is not automatically backfilled. Enquiry delivery mode does not disable monthly reporting. Tests use real previous-month data, go only to the saved test address, and display “TEST — OUT-OF-SEQUENCE” in the subject and body.
