...
function costReport(result, compact = false) {
  ...
  if (result.unavailable || !result.properties) return notice + empty('Billing figures are not available yet. They will appear automatically when Azure allows the next update.');
  ...
  const summary = `<div class="metrics">${metric('Month to date',money(total),'Reported actual cost')}${metric('Month-end estimate',money(daily===null ? null : daily*monthDays))}${metric('Next 6 months',money(daily===null ? null : daily*365.25/2),'At current daily rate')}${metric('Next 12 months',money(daily===null ? null : daily*365.25),'At current daily rate')}</div><p class="footnote">${esc(currency)} · Join System subscription · Updated ${esc(time(result.fetchedAt))}. Billing refreshes at most every six hours; Azure reporting is delayed.</p>`;
  ...
  return notice + summary + panel('Cost by Azure service',rows.length ? table(['Service',`Cost (${currency})`],[...services].sort((a,b)=>b[1]-a[1]).map(([name,value])=>`<tr><td>${esc(name)}</td><td>${esc(money(value))}</td></tr>`)) : empty('No billed usage returned.')) + panel('How estimates work','<p>Estimates extend the average daily spend from completed days this month. They exclude future changes in traffic, pricing and resources. Billing delays can understate estimates, especially early in the month.</p>');
}
