...
function costReport(result, compact = false) {
  ...
  const summary = `<div class="metrics">${metric('Month to date',money(total),'Reported actual cost')}${metric('Month-end estimate',money(daily===null ? null : daily*monthDays))}${metric('Next 6 months',money(daily===null ? null : daily*365.25/2),'At current daily rate')}${metric('Next 12 months',money(daily===null ? null : daily*365.25),'At current daily rate')}</div><p class="footnote">${esc(currency)} · Club Directory system · Updated ${esc(time(result.fetchedAt))}. Billing refreshes at most every six hours; billing data can be delayed.</p>`;
